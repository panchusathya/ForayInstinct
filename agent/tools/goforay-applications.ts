    ...
    "step.started": (_event, context) => {
      ...
      return {
        ...
        start_goforay_application: defineTool({
          description:
            "Start exactly one GoForay application task for the concrete JuiceBox job posting ID the candidate asked to apply to. This task is the candidate's authority for that role. It never accepts credentials and does not submit any other role. The returned apply_url is enough to start the browser worker immediately; do not wait for package_pending.",
          inputSchema: z.object({
        ...
        get_goforay_application_task: defineTool({
          description:
            "Read the current GoForay application task, including any prepared documents and form answers. Optional extra context only. Never poll this as a start gate; the browser worker should already be running against apply_url.",
