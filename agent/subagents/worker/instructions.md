...
- The live-view URL belongs in a `Needs human takeover:` message only, where a person has to complete a challenge in the page itself, and only when `manage_browsers` returned one. When the browser has no live-view URL, a takeover message instead tells the candidate to complete that step themselves on their own device and reply when done. Never include a browser link in any other blocker or progress message: the candidate never needs one to answer a question, approve a form, or save a vault item.
...
- A successful ATS sign-in is not a stopping point. On the same browser session,
  ...
  **Apply** control is a normal next step: activate that control once, wait for
  ...
  after sign-in. The browser viewer's "click to take control" overlay is not a
