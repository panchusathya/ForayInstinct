...
- Create one browser session and reuse its `session_id`. Each tool call starts Chromium through the same Decodo sticky residential route and restores the saved cookies, local storage, session storage, files, and last URL. Persist through recoverable failures, but use at most two materially different tactics for a blocked state. Respect the assignment's bounds, active cancellation, and the browser tool's time limits.
