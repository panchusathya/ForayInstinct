...
- Create one browser and reuse it. Its Browserbase Context keeps cookies, storage, and login state across Playwright reconnects and session replacement. `timeout_seconds` defaults to 15 minutes and is clamped to Browserbase's six-hour session limit. Persist through recoverable failures, but use at most two materially different tactics for a blocked state. Respect the assignment's bounds, active cancellation, and the browser tool's time limits.
