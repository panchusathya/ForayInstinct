...
const currentState = async () => {
  ...
  if (await visible(page.locator('[data-automation-id="applyFlowPage"], [data-automation-id="pageFooterNextButton"], [data-automation-id="progressBar"], [data-automation-id="bottom-navigation-next-button"], input[data-automation-id="legalNameSection_firstName"]'))) {
    return { state: "wizard_ready", url };
  }
  return null;
};

await page.goto(applicationUrl, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => undefined);
...
await click(page.getByRole("button", { name: /accept cookies|accept all/i })).catch(() => undefined);
...
for (let attempt = 0; attempt < 4; attempt += 1) {
  ...
  if (await click(page.getByRole("button", { name: /^continue application$/i }))) continue;
  if (await click(page.getByRole("link", { name: /^continue application$/i }))) continue;
  if (await click(page.locator('[data-automation-id="applyManually"]'))) continue;
  if (await click(page.getByRole("button", { name: /^apply manually$/i }))) continue;
  if (await click(page.getByRole("link", { name: /^apply manually$/i }))) continue;
  if (await click(page.locator('button[data-automation-id="SignInWithEmailButton"]'))) continue;
  if (await click(page.getByRole("button", { name: /^sign in with email(?: address)?$/i }))) continue;
  if (await click(page.getByRole("button", { name: /^apply$/i }))) continue;
  if (await click(page.getByRole("link", { name: /^apply$/i }))) continue;
  break;
}

const state = await currentState();
return state ?? { state: "route_incomplete", url: page.url() };
`;
}
...
function isRouteResult(value: unknown): value is WorkdayRouteResult {
  ...
  const candidate = value as { state?: unknown; url?: unknown };
  return (
