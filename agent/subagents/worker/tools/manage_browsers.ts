...
function lifecycleResult(
  ...
  return {
    ...
    next_actions: [
      ...
      `For a third-party ATS page, inspect the returned page URL and post-action browser state with session_id "${value.session_id}" first. Call computer_action with a screenshot only when visual inspection is required (coordinate targeting, a captcha grid, or an ambiguous overlay) before execute_playwright_code. Do not assume an iframe or #resume.`,
      ...
      `Use computer_action with session_id "${value.session_id}" for coordinate-level input, or a screenshot when the DOM is not enough to target the next control.`,
